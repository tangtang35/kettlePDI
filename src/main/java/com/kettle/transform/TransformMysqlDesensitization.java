...
public class TransformMysqlDesensitization {

    private static PluginRegistry pluginRegistry = PluginRegistry.getInstance();
    //存储获取的字段
    private static String[] filedNames = {"EMAIL","IP"};
    ...
    public static DatabaseInfo buildDatabaseInfo(DatabaseInfo databaseInfo, Boolean srcOrTar) {
        if(srcOrTar){
            ...
            databaseInfo.setHostName("172.24.1.127");
            databaseInfo.setPort("3306");
            databaseInfo.setDbName("test");
            databaseInfo.setDisplayName("172.24.1.127:3306/test");
            databaseInfo.setPassword("Ankki_mySQL123");
            databaseInfo.setSupportBooleanDataType(true);
            databaseInfo.setSupportTimestampDataType(true);
            databaseInfo.setUsername("root");
            databaseInfo.setSchemaName("");
            databaseInfo.setTableName("USER_INFO");
            databaseInfo.setSql("select * from USER_INFO_1");
            ...
            databaseInfo.setName("test1");
            databaseInfo.setDatabaseType("mysql");
            databaseInfo.setAccessType(0);
            databaseInfo.setHostName("172.24.1.127");
            databaseInfo.setPort("3306");
            databaseInfo.setDbName("test1");
            databaseInfo.setDisplayName("172.24.1.127:3306/tset1");
            databaseInfo.setPassword("Ankki_mySQL123");
            databaseInfo.setSupportBooleanDataType(true);
            databaseInfo.setSupportTimestampDataType(true);
            databaseInfo.setUsername("root");
            databaseInfo.setSchemaName("");
            databaseInfo.setTableName("USER_INFO_NEW");
            Map<String,String> map = new HashMap<String, String>();
            map.put("SUPPORTS_BOOLEAN_DATA_TYPE","Y");
            map.put("SUPPORTS_TIMESTAMP_DATA_TYPE","Y");
            databaseInfo.setProperty(map);
    ...
    public TransMeta getTransMeta(DatabaseInfo srcDatabaseInfo,DatabaseInfo tarDatabaseInfo) {
        ...
        transMeta.addTransHop(new TransHopMeta(dataProcessStep,outputStep));
        return transMeta;
    }
    ...
    public StepMeta createDataProcessTransMeta(TransMeta transMeta,DatabaseInfo databaseInfo) {
        ...
        fieldDataType.put("EMAIL","随机");
        fieldDataType.put("IP","映射");
        dataProcess.setFieldDataType(fieldDataType);
        dataProcess.setDbType("mysql");
